...
from keras.callbacks import EarlyStopping,Callback # Note: Callback import is no longer strictly needed but kept for context
...
# --- CALLBACKS & HELPERS ---
# F1History Custom Callback REMOVED to prevent retracing warnings.
...
def cross_val_mlp(X, y, n_splits=5, epochs=55, batch_size=32, patience=8): 
    """
...
# --- PLOTTING FUNCTION (Modified to remove F1 plot) ---
def plot_all_metrics(history, y_true, y_pred, model_name="MLP Model"):
    """ Plots Accuracy and the Confusion Matrix in a single figure. 
        Note: F1 score plot removed since the F1History callback is removed.
    """
    ...
    disp.plot(cmap=plt.cm.Blues, ax=ax[1], values_format='d', colorbar=False)
    ax[1].set_title("Test Set Confusion Matrix", fontsize=12)
    
    plt.tight_layout()
    plt.show()


# --- FINAL MODEL TRAINING & EVALUATION (Updated Callbacks) ---
def KerasMLP():
    """
    ...
    history = mlpModel.fit(
        ...
        epochs=55,
        batch_size=32,
        callbacks=[early_stop], # Only using EarlyStopping
        verbose=1
    )
